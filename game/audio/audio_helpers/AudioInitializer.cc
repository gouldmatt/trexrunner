 ...
 *      Implementation of IicConfig, AudioPllConfig, AudioWriteToReg, LineInLineOutConfig
 *      InitializeAudio, readButtons and clearRegister
 */
...
int readButtons(XGpio input){
	...
	while(count < 150000){
		...
		//wait until button_data is stable
		if(last == button_data){
			count ++;
		} else {
			count = 0;
...
/*
 * clearRegisters() - clears state shared memory addresses
 */
void clearRegisters(){
